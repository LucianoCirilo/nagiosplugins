...
then
   echo ""
   echo "./check_ping_host.sh  [FQDN]" 
   echo ""
   echo "Exemplo:" 
   echo "" 
   echo "./check_ping_host.sh  www.google.com.br" 
   echo ""
   exit 1
fi
...
ping -qc3 "$1" > /tmp/check-ping-host$1 2>&1 > /dev/null
a=$?
ping -qc3 "$1" >> /tmp/check-ping-host$1 2>&1 > /dev/null
b=$?
ping -qc3 "$1" >> /tmp/check-ping-host$1 2>&1 > /dev/null
...
then
	echo "Esta OK"
	exit 0
elif cat /tmp/check-ping-host-$1 | grep unknown 
then
